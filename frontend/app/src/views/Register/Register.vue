...
<div class="register-container">
    ...
    <div class="bx--grid bx--grid--full-width home">
        ...
        <cv-toast-notification v-if="showErrorMessage" kind="error" :title="errorMessage" :low-contrast="true"></cv-toast-notification>
        <br>
        <cv-form onsubmit="event.preventDefault();">
            <cv-text-input style="float:left;" class="firstName" label="First name" helper-text="" v-model="firstName" placeholder="Jane">
                <template v-if="!firstNameValid" slot="invalid-message">A first name is required</template>
            </cv-text-input>
            <cv-text-input style="float:right;" class="lastName" label="Last name" helper-text="" v-model="lastName" placeholder="Doe">
                <template v-if="!lastNameValid" slot="invalid-message">A last name is required</template>
            </cv-text-input>
            <br>
            <cv-text-input class="email" label="Email" helper-text="" v-model="email" placeholder="example@example.com">
                <template v-if="!emailValid" slot="invalid-message">A valid email is required</template>
            </cv-text-input>
            <cv-text-input class="password" label="Password" helper-text="" placeholder="" v-model="password" type="password" passwordHideLabel="Hide password" passwordShowLabel="Show password">
            ...
            <cv-text-input class="confirmPassword" label="Confirm password" helper-text="" placeholder="" v-model="confirmPassword" type="password" passwordHideLabel="Hide password" passwordShowLabel="Show password">
                <template v-if="!confirmPasswordValid" slot="invalid-message">Please make sure passwords match</template>
            </cv-text-input>
            <br>
...
export default {
    ...
    methods: {
        ...
        register() {
            ...
            if (this.passwordValid && this.emailValid && this.confirmPasswordValid && this.firstNameValid && this.lastNameValid) {
                axios.post('https://api.rpsh.me/v1/user/signup', {
                        email: this.email,
