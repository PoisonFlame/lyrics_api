    ...
    <div class="bx--grid bx--grid--full-width home">
        ...
        <cv-form onsubmit="event.preventDefault();">
            ...
            <cv-text-input class="password" label="Password:" helper-text="" placeholder="" v-model="password" type="password" passwordHideLabel="Hide password" passwordShowLabel="Show password">
                <template v-if="!passwordValid" slot="invalid-message">A valid password is required</template>
            </cv-text-input>
            <br>
            <cv-button kind="primary" class="btn_login" @click="login">
                Login
            </cv-button>
            <cv-button kind="secondary" class="btn_register" @submit.prevent="register" @click="register">
                Register
            </cv-button>
        </cv-form>
    </div>
    ...
    methods: {
        login() {
            ...
            if (this.passwordValid && this.emailValid) {
                axios.post('https://api.rpsh.me/v1/user/login', {
                        email: this.email,
